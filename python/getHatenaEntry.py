...
def get_collection_uri(hatena_id, blog_id, password):
    service_doc_uri = "https://blog.hatena.ne.jp/{hatena_id:}/{blog_id:}/atom".format(
        hatena_id=hatena_id, blog_id=blog_id)
    # リクエストを送る
    res_service_doc = requests.get(
        url=service_doc_uri, auth=(hatena_id, password))
    # 問題なければコレクションURIを返す
    if res_service_doc.ok:
        soup_servicedoc_xml = bs4.BeautifulSoup(
            res_service_doc.content, features="xml")
...
for i in range(MAX_ITERATER_NUM):
    ...
    # Beatifulsoup4でDOM化
    soup_collectino_xml = bs4.BeautifulSoup(
        res_collection.content, features="xml")
    # 記事のノードリスト取得
